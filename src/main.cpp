...
{
  std::vector<std::string> splittedStrings2 = split(splittedStrings[1], "beac");

  if (!splittedStrings2.empty() && splittedStrings2.size() > 1) 
  {
    if (splittedStrings2[0] == manufacturerAndroid.c_str())
    {
      String device = advertisedDevice.toString().c_str();
      Serial.println("Dispositivo Android detectado");
      std::vector<std::string> splittedStrings3 = split(splittedStrings2[1], finalId.c_str());

      if (!splittedStrings3.empty() && splittedStrings3.size() > 1) {
        std::vector<std::string> splittedStrings4 = split(splittedStrings3[1], "c5");
        std::vector<std::string> splittedStringsTime = split(splittedStrings3[0], initialId.c_str());
        std::vector<std::string> timeSplitted = separatestring(splittedStringsTime[1], 3);
        std::string timeFinal = timeSplitted[0] + ":" + timeSplitted[1] + ":" + timeSplitted[2];
        ////Serial.println(timeFinal.c_str());
        formattedDate = timeFinal.c_str();
        if (!splittedStrings4.empty() && splittedStrings4.size() > 1) {
          char *cstr = &splittedStrings4[0][0];
          std::vector<std::string> hexStrings = separatestring(splittedStrings4[0], 2);
          mId = hexStrings[0];
          String codeAndroid; 
          codeAndroid = strtol(hexStrings[0].c_str(), NULL , 16);
          miId = hexStrings[1];
          hexStrings.clear();
          
          String macAddressAndroid = device.substring(device.indexOf("Address: ")+9,device.indexOf("Address: ")+26);
          macAddressAndroid.toUpperCase();

          registraDadosDoUsuario(macAddressAndroid, codeAndroid, advertisedDevice.getRSSI(), 2, 0, 0, 0, 0, 0);
        }
      }
    }
  }
...
void GetSmartPhoneIOSInfo(std::vector<std::string> splittedStrings, BLEAdvertisedDevice advertisedDevice)
{
  String temp = splittedStrings[1].c_str();
  if(temp.substring(4,8) == "0215")
  {
    std::vector<std::string> splittedStrings2 = split(splittedStrings[1], "0215");

    if (!splittedStrings2.empty() && splittedStrings2.size() > 1) 
    {
      if (splittedStrings2[0] == manufacturerIOS.c_str())
      {
        Serial.println("Dispositivo IOS detectado");
        String device = advertisedDevice.toString().c_str();
        std::vector<std::string> splittedStrings3 = split(splittedStrings2[1], finalId.c_str());
        ...
        if (!splittedStrings3.empty() && splittedStrings3.size() > 1) {
          ...
          if (!splittedStrings4.empty()) {
            char *cstr = &splittedStrings4[0][0];
            std::vector<std::string> hexStrings = separatestring(splittedStrings4[0], 2);
            mId = hexStrings[0];
            String codeIOS;
            codeIOS = strtol(hexStrings[0].c_str(), NULL , 16);
            miId = hexStrings[1];
            hexStrings.clear();
            
            String macAddressIOS = device.substring(device.indexOf("Address: ")+9,device.indexOf("Address: ")+26);
            macAddressIOS.toUpperCase();

            registraDadosDoUsuario(macAddressIOS, codeIOS, advertisedDevice.getRSSI(), 2, 0, 0, 0, 0, 0);
          }
        }
      }
    }           
...
void GetSmartPhoneInfos(BLEAdvertisedDevice advertisedDevice)
{
  std::string s = advertisedDevice.toString().c_str();

  std::vector<std::string> splittedStrings = split(s, "manufacturer data: ");

  if (!splittedStrings.empty() && splittedStrings.size() > 1 ) 
  {
      GetSmartPhoneAndroidInfo(splittedStrings, advertisedDevice);      
      GetSmartPhoneIOSInfo(splittedStrings, advertisedDevice);
  }
}
